...
> 原文：[https://www.bookbookmark.ds100.org/ch/07/web_http.html](https://www.bookbookmark.ds100.org/ch/07/web_http.html)
...
http（aka**h**yper**t**ext**t**t ransfer**p**rotocol）是一种 _ 请求-响应 _ 协议，允许一台计算机通过 Internet 与另一台计算机对话。

## 请求和响应

互联网允许计算机互相发送文本，但不会对文本包含的内容施加任何限制。HTTP 定义了一台计算机（客户端）和另一台计算机（服务器）之间的文本通信结构。在这个协议中，客户机向服务器提交一个 _ 请求 _，一个特殊格式的文本消息。服务器将文本 _ 响应 _ 发送回客户端。
...
运行上面的`curl`命令会导致客户机的计算机构造一条如下所示的文本消息：
...
此消息遵循特定的格式：以`GET /html HTTP/1.1`开头，表示消息是对`/html`页的 HTTP`GET`请求。表单 HTTP 头后面的三行中的每一行，都是`curl`发送给服务器的可选信息。HTTP 头的格式为`{name}: {value}`。最后，消息结尾的空白行告诉服务器消息在三个头之后结束。注意，我们在上面的代码段中用`{blank_line}`标记了空白行；在实际的消息`{blank_line}`中用空行替换。
...
几乎每一个与因特网交互的应用程序都使用这个 HTTP 协议。例如，在 Web 浏览器中访问[https://httpbin.org/html](https://httpbin.org/html)会发出与上面的`curl`命令相同的基本 HTTP 请求。您的浏览器没有像上面那样将响应显示为纯文本，而是识别出文本是一个 HTML 文档，并将相应地显示它。

实际上，我们不会以文本形式写出完整的 HTTP 请求。相反，我们使用像`curl`或 python 库这样的工具来构造请求。

### 在 python 中[？](#In-Python)

python**请求**库允许我们用 python 发出 HTTP 请求。下面的代码发出与运行`curl -v https://httpbin.org/html`相同的 HTTP 请求。
...
HTTP 响应包含一个状态代码，一个特殊的数字，指示请求是成功还是失败。状态代码`200`表示请求成功。
...
`GET`请求用于从服务器检索信息。由于 Web 浏览器每次在地址栏中输入 URL 时都会发出`GET`请求，`GET`请求是最常见的 HTTP 请求类型。
...
服务器将用状态代码响应，以指示`POST`请求是否成功完成。此外，服务器通常会发送一个响应主体来显示给客户机。
...
## 响应状态代码类型[¶](#Types-of-Response-Status-Codes)

先前的 HTTP 响应具有 HTTP 状态代码`200`。此状态代码表示请求已成功完成。还有数百个其他的 HTTP 状态代码。谢天谢地，它们被分为不同的类别，以便于记忆：

*   **100s**-信息：需要来自客户端或服务器 _ 的更多输入（例如 100 continue、102 processing）_
*   **200S**-成功：客户端请求成功 _（例如 200 OK，202 accepted）_
*   **300s**-重定向：请求的 URL 位于其他位置；可能需要用户的进一步操作 _（例如，300 多个选项，301 永久移动）_
*   **400s**-客户端错误：客户端错误 _（例如 400 错误请求，403 禁止，404 未找到）_
*   **500s**-服务器错误：服务器端错误或服务器无法执行请求 _（例如，500 内部服务器错误，503 服务不可用）_
